...
def elo_leaderboard(
    ...
    top_n: int | None = 50,
    min_elo_fights: int = 0,
) -> pd.DataFrame:
    """
    Rank fighters by their latest available pre-fight Elo snapshot.

    Args:
        fighter_stats: cleaned["fighter_stats"].
        top_n: optional number of rows to return. Use None for all rows.
        min_elo_fights: minimum prior Elo-tracked fights required.
    """
    required_cols = {"fighter_url", "date", "bout_url", "elo_pre", "elo_fights"}
    missing_cols = required_cols - set(fighter_stats.columns)
    if missing_cols:
        raise ValueError(f"fighter_stats is missing required columns: {sorted(missing_cols)}")

    leaderboard = (
        fighter_stats
        .sort_values(["fighter_url", "date", "bout_url"])
        .groupby("fighter_url", as_index=False)
        .tail(1)
        .copy()
    )

    leaderboard = leaderboard[leaderboard["elo_fights"] >= min_elo_fights]
    leaderboard = leaderboard.sort_values("elo_pre", ascending=False).reset_index(drop=True)
    ...
    leaderboard = leaderboard[display_cols]
